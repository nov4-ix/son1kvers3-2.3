...
const SmartPaymentSelector = () => {
    ...
    // Simular detección de ubicación (idealmente usar geolocalización real o headers)
    useEffect(() => {
        const detectLocation = async () => {
            try {
                ...
                // Auto-seleccionar método según región
                if (isLatam) {
    ...
    const handleStripeSubscribe = async (plan: any) => {
    ...
    const handlePayPalApprove = async (data: any, actions: any) => {
        // Here we might just verify or let the webhook handle it
        // Optionally notify backend immediately
    ...
    const initialPayPalOptions: ReactPayPalScriptOptions = {
        clientId: import.meta.env.VITE_PAYPAL_CLIENT_ID || "sb", // sb = sandbox default
        vault: true,
        intent: 'subscription',
    };
                ...
                <div className="max-w-2xl mx-auto mb-12">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        {paymentMethods.map((method) => (
                            <button
                                key={method.id}
                                onClick={() => setSelectedMethod(method.id as 'stripe' | 'paypal')}
                        ...
                        {plans.map((plan) => (
                            ...
                            >
                                ...
                                <div className="mt-auto">
                                    {(selectedMethod === 'stripe' || !plan.paypalPlanId) ? (
                                        <button
                                        ...
                                        >
                                            ...
                                            ) : plan.stripePriceId || (plan.paypalPlanId && selectedMethod === 'paypal') ? (
                                                'Suscribirme'
                                            ) : (
